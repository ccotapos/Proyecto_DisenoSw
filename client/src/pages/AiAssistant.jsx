...
const AiAssistant = () => {
  ...
  const handleChatSubmit = async (e) => {
    ...
    try {
      ...
      setChatHistory(prev => [...prev, { type: 'ai', text: t('ai.error_chat') }]);
    } finally {
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <p className="text-center text-gray-500 mb-10">{t('ai.subtitle')}</p>
            ...
            {chatHistory.map((msg, index) => (
              <div key={index} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-4 rounded-2xl text-sm whitespace-pre-wrap leading-relaxed shadow-sm ${
                  msg.type === 'user' ? 'bg-brand-primary text-white rounded-tr-none' : 'bg-white text-gray-800 border border-gray-200 rounded-tl-none'
                }`}>
                  {msg.text}
                </div>
              </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg border-t-4 border-blue-600">
            ...
            <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-blue-300 border-dashed rounded-lg cursor-pointer bg-blue-50 hover:bg-blue-100 transition ${analyzingPdf ? 'opacity-50 cursor-not-allowed' : ''}`}>
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                <p className="text-3xl mb-2">{analyzingPdf ? t('ai.pdf_loading_icon') : '📤'}</p>
